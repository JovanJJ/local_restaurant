...
export default function Navbar() {
  ...
  return (
    <div className="absolute top-5 md:top-8 left-0 right-0 z-30 flex justify-center pointer-events-none px-5">
      <HeaderEntrance className="w-full max-w-6xl flex flex-col items-center gap-5 pointer-events-auto lg:flex-row lg:justify-between">
        <div className="flex items-center gap-8">
          <Link href={`/${lang}`} className="relative group transition-all duration-300">
            ...
            <Image
              ...
              height={68}
              priority
              className="relative h-12 md:h-16 w-auto object-contain transition-all duration-500 ease-out group-hover:scale-105"
            />
          </Link>
        ...
        <nav className="flex flex-wrap items-center justify-center gap-x-5 gap-y-3 font-sans text-[10px] font-semibold uppercase tracking-[0.22em] text-[#E8DCCF] sm:gap-x-8 sm:text-xs">
          {navLinks.map((link) => {
